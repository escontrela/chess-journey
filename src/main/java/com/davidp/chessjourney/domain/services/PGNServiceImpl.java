...
/**
 * This class is responsible for implements the service for managing the PGN format and conversions
 * between algebraic coordinates and standard algebraic notation.
 *
 * <p><a href="https://en.wikipedia.org/wiki/Portable_Game_Notation">...</a>
 */
public class PGNServiceImpl implements PGNService {

  public static final String PGN_REGEXPR =
  ...
  /**
   * This method converts algebraic coordinates (e.g., e2, e4) into standard algebraic notation
   * (e.g., e4, Nf3) Example: c2 e5 : Ne5
   *
  ...
  public String toAlgebraic(
    ...
    ChessBoard boardCopy = ChessBoardFactory.createFromFEN(board.getFen());
    if (chessRules.isValidMove(from, to, boardCopy.getFen())) {

      boardCopy.movePiece(movingPiece.getPiece(), from, to);
      boardCopy.setTurn(movingPiece.getPiece().getColor().opposite());
      System.out.println("Fen position after move:"+boardCopy.getFen().getStringValue());
      if (chessRules.isCheckOrMate(boardCopy.getFen())) {

        if (chessRules.isCheck(boardCopy.getFen())) {
          moveNotation.append('+');
        } else if (chessRules.isMate(boardCopy.getFen())) {
          moveNotation.append('#');
        }

        //TODO CHES IF IS STALE MATE OR DRAW FOR SEVERAL REASONS
      }
    }
  ...
  public enum PGNRegExprGroups {
    CASTLING_GROUP_1(1),
    CASTLING_CHECK_OR_MATE_GROUP_2(2),
    PIECE_GROUP_3(3),
    DISAMBIGUATION_GROUP_4(4),
    CAPTURE_GROUP_5(5),
    DESTINATION_GROUP_6(6),
    PROMOTION_GROUP_7(7),
    CHECK_OR_MATE_GROUP_8(8);
   ...
   */
  public static Map<PGNRegExprGroups, String> extractValuableGroups(
      String notation, Pattern pattern, PGNRegExprGroups... relevantGroups) {
   ...
   */
  public static Map<PGNRegExprGroups, String> extractAllValuableGroups(
      String notation, Pattern pattern) {
  ...
  // INMO this method do not have to validate the move, it's the chess rules that must do it, only
  // should transform the notation to a list of positions

  /**
   * This method converts standard algebraic notation (e.g., e4, Nf3) into algebraic coordinates
   * (e.g., e2, e4) to allow moves to be made on the board.
   *
  ...
  public GameMove fromAlgebraic(String move, ChessBoard board) {
    ...
    var activeColor =
        gameStatue.getActiveColor(); // necesario para identificar si movemos blancas o negras.

    // TODO sin entran variantes no standard no ajustar la expresión regular, mejor convertir esas
    // variantes a standard
    ...
    // Nf3-> grupo 3 & grupo 6 --> ES más complicado , porque hay que tener en cuenta que no estén
    // otros grupos por el medio....
    // Test if the move is a castling
    if (groups.containsKey(PGNRegExprGroups.PIECE_GROUP_3)
        && groups.containsKey(PGNRegExprGroups.DESTINATION_GROUP_6)) {
      ...
      List<Pos> possiblePositions =
          board.getAllPiecePositionsOfType(
              mapPGNLetterToPieceType(piece.toLowerCase()), activeColor);
      ...
      toret =
          possiblePositions.stream()
              ...
              .map(
                  p ->
                      GameMoveFactory.createNormalMove(
    ...
    if (groups.containsKey(PGNRegExprGroups.DESTINATION_GROUP_6)
        && Stream.of(
                PGNRegExprGroups.PIECE_GROUP_3,
                PGNRegExprGroups.DISAMBIGUATION_GROUP_4,
                PGNRegExprGroups.CAPTURE_GROUP_5)
      ...
      toret =
          possiblePawnPositions.stream()
              ...
              .map(
                  p ->
                      finalIsPromotion
                          ? GameMoveFactory.createPromotionMove(
                              new BoardMove(p, destinationPos),
                              finalPromotionPiece,
                              false,
                              finalIsCheck,
